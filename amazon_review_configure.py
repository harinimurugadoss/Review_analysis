...
class AmazonFullScraper:
    ...
    def smart_sleep(self):
        ...
        base_delay = random.uniform(7, 9)
        if random.random() < 0.1:  # 10% chance of longer delay
            base_delay += random.uniform(10, 14)
        time.sleep(base_delay)

    def scrape_reviews(self, url, max_reviews=1000):
    ...
    def save_to_csv(self, reviews_data, filename):
        if not reviews_data:
...
def main():
    # User-provided item name and count of review comments
    url = input("Enter the Amazon product URL: ")
    item_name = input("Enter the item name for saving: ")
    ...
    if reviews:
        # Save file using item name
        filename = f"Samsung_adaptor_reviews_03.csv"
